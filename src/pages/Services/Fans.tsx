...
import { Heart, Users, Video, Star } from 'react-feather';
import { websiteInfo } from '../../data/website/info';
import { useLanguage } from '../../contexts/LanguageContext';
import { colors } from '../../data/colors/theme';
// Import language files
import enFans from '../data/text/en/fans.json';
import zhFans from '../data/text/zh/fans.json';
import jaFans from '../data/text/ja/fans.json';
import esFans from '../data/text/es/fans.json';
...
const Fans: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setFormData({ name: '', email: '', team: '', sport: '' });
  };
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Become a Fan Section with Form */}
        ...
        <div className="max-w-5xl mx-auto">
          ...
          >
            <div className="flex justify-center mb-6">
              <Heart size={40} color={colors.primaryColor1} />
            </div>
            <h2 className="text-3xl font-bold mb-6" style={{ color: colors.primaryColor1 }}>
              {processText(pageContent.becomeFan.title)}
            </h2>
            <p className="text-lg text-gray-600 max-w-3xl mx-auto mb-12">
              {processText(pageContent.becomeFan.description)}
            </p>
            {/* Sign-Up Form */}
            <motion.div
              initial={{ opacity: 0, y: 20 }}
            ...
            >
              <h3 className="text-2xl font-bold mb-4" style={{ color: colors.primaryColor1 }}>
                {processText(pageContent.becomeFan.form.title)}
              </h3>
              <p className="text-gray-600 mb-6">{processText(pageContent.becomeFan.form.subtitle)}</p>
              <div
                className="space-y-6"
              ...
              >
                <div>
                  <label className="block text-gray-700 mb-2" htmlFor="name">
                    {processText(pageContent.becomeFan.form.fields.name)}
                  </label>
                  <input
                    type="text"
                    id="name"
                ...
                <div>
                  <label className="block text-gray-700 mb-2" htmlFor="email">
                    {processText(pageContent.becomeFan.form.fields.email)}
                  </label>
                  <input
                    type="email"
                    id="email"
                ...
                <div>
                  <label className="block text-gray-700 mb-2" htmlFor="team">
                    {processText(pageContent.becomeFan.form.fields.team)}
                  </label>
                  <input
                    type="text"
                    id="team"
                ...
                <div>
                  <label className="block text-gray-700 mb-2" htmlFor="sport">
                    {processText(pageContent.becomeFan.form.fields.sport)}
                  </label>
                  <select
                    id="sport"
                    name="sport"
                  ...
                  >
                    ...
                    {pageContent.becomeFan.form.sports.map((sport) => (
                ...
                >
                  {processText(pageContent.becomeFan.form.fields.submit)}
                </button>
              </div>
            </motion.div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {pageContent.becomeFan.steps.map((step, index) => (
                <motion.div
                  key={step.id}
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                ...
                >
                  <div className="flex flex-col items-center">
                    ...
                    >
                      {iconComponents[step.icon] || <Heart size={24} color={colors.primaryColor1} />}
                    </div>
      ...
      <section className="py-20 px-6 bg-gray-50">
        <div className="max-w-6xl mx-auto">
          ...
          >
            {processText(pageContent.fanFeatures.title)}
          </motion.h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {pageContent.fanFeatures.items.map((feature, index) => (
              <motion.div
                key={feature.id}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                ...
                <div className="flex flex-col items-center">
                  {feature.image ? (
                    ...
                    <div
                      ...
                      style={{ backgroundColor: `${colors.primaryColor1}20` }}
                    >
                      {iconComponents[feature.icon] || <Star size={32} color={colors.primaryColor1} />}
                    </div>
                  )}
                  <h3 className="text-xl font-bold mb-2" style={{ color: colors.primaryColor1 }}>
                    {feature.title}
              ...
              >
                <div className="flex flex-col items-center">
                  <div
                    ...
                    style={{ backgroundColor: `${colors.primaryColor1}20` }}
                  >
                    {iconComponents[benefit.icon] || <Heart size={32} color={colors.primaryColor1} />}
                  </div>
                  <h3 className="text-xl font-bold mb-2" style={{ color: colors.primaryColor1 }}>
                    {benefit.title}
                  </h3>
